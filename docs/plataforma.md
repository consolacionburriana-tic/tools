...
| Licencias digitales | ✅ | ✅ | ✅ (Fases 0-3 + 5: campaña completa de punta a punta — pedido a editoriales de pago **y** del banco de libros (dos pedidos separados), **envío de los códigos a cada alumno desde la app** (pegar la columna del Excel de la editorial, asignación en orden, sobrantes, correo por licencia), seguimiento 🧾/📤/💰, cobro y esquema del proceso; informes verificados contra Neon. Los pedidos salen como Google Sheets por editorial en Drive, con PDF. `licencias-envios.sql` aplicado en Neon) | [`11-licencias-v2.md`](./11-licencias-v2.md) |
...
| Evaluaciones de actividades | ✅ | ✅ | ✅ (Fases 0-4 en producción: editor con presets, formulario público, envío por correo (Resend, también programado), dashboard y comparativas · evaluación conjunta alumnado + profesorado + familias) | [`16-evaluaciones.md`](./16-evaluaciones.md) |
...
| Alumnado (ficha por alumno) | ✅ | ✅ | 🟡 (navegador por clase, buscador y ficha completa con los 8 módulos, todo copiable de un toque; alcance por etapa verificado contra la app. Pestañas por clase de protección de datos —un check general sí/no que despliega imagen, redes, AMPA y ONG, más la desestimación del correo del alumno—, banco de libros, AMPA y **venta de materiales** (columnas creadas por secretaría/dirección/TIC, pagado/no/becado/no aplica), e **informes a medida en PDF, Excel y CSV**. SQL aplicado en Neon, incluida la retirada de `pd_firmada`) | [`21-alumnado.md`](./21-alumnado.md) |
...
Sheets. Cubre la campaña entera (Fases 0-3 y 5): recoger pedidos, **pedir a las editoriales** (dos
informes: las de pago, incremental, y las gratis del banco de libros, censo completo),
**mandarle a cada alumno su código** desde `/gestion/licencias/envios` (se pega la columna del
Excel de la editorial, se reparte en orden, los que sobran quedan en el almacén, y sale un correo
por licencia marcándolas una a una) y cobrar con el CSV de Educamos, con seguimiento 🧾/📤/💰 por
pedido. Quien se pierda tiene el **esquema del proceso** en `/gestion/licencias/proceso`. Queda el
