...
const tools = [
  {
    ...
    icon: ClipboardList,
    title: 'Registro ABC',
    description: 'Registrar conductas disruptivas de alumnos con necesidades educativas especiales.',
    badge: 'Público',
  },
];
    ...
    <main className="flex-1 container max-w-2xl mx-auto px-4 py-12 space-y-10">
      <header className="space-y-2">
        ...
        <p className="text-zinc-500 dark:text-zinc-400 text-base">
          Herramientas internas del Colegio Consolación · Burriana
        </p>
      </header>
        ...
        <div className="grid gap-3">
          {tools.map((tool) => (
            <Link
              key={tool.href}
              href={tool.href}
              className="group flex items-start gap-4 p-5 rounded-2xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 hover:border-teal-300 dark:hover:border-teal-700 transition-colors duration-150 shadow-xs hover:shadow-sm"
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-0.5 leading-snug">{tool.description}</p>
              </div>
            </Link>
          ))}
        </div>
