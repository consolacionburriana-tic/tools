...
export function valoresClase(clase: ClaseCuaderno, tutor: TutorCuaderno | null, numAlumnos: number): Valores {
  ...
  return {
    ...
    tutor_completo: tutor?.completo ?? tutor?.nombre ?? '',
    tutor_nombre: tutor?.pila ?? '',
    tutor_apellido1: tutor?.apellido1 ?? '',
    tutor_apellido2: tutor?.apellido2 ?? '',
    tutor_1apellido: [tutor?.pila, tutor?.apellido1].filter(Boolean).join(' '),
