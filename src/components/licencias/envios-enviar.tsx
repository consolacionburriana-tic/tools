...
export function EnviosEnviar({ abierto, onCerrar, tipo, destino, seleccionadas, presets, onHecho }: Props) {
  ...
  const correos = useMemo(() => {
    if (!fusionar) return seleccionadas.length;
    return new Set(seleccionadas.map((l) => `${l.studentId}|${l.destinatario}`)).size;
  }, [seleccionadas, fusionar]);
  ...
  async function enviar() {
    const aviso =
      `Se van a mandar ${correos} correo(s) con ${seleccionadas.length} licencia(s).` +
      (correos > AVISO_CUOTA
        ? `\n\n⚠️ Son más de ${AVISO_CUOTA} en una tacada y Gmail corta sobre los 2.000 al día por buzón. Conviene fusionar por alumno o partirlo en dos días.`
        : '') +
      '\n\n¿Seguimos?';
    ...
    if (fusionar) {
      ...
      for (let i = 0; i < seleccionadas.length; i += TANDA) {
        tandas.push(seleccionadas.slice(i, i + TANDA).map((l) => l.id));
      }
    }
  ...
  return (
    <Dialog open={abierto} onOpenChange={(o) => (o ? null : onCerrar())}>
      <DialogContent className="max-h-[92vh] w-full max-w-[min(64rem,calc(100%-2rem))] overflow-y-auto sm:max-w-[min(64rem,calc(100%-2rem))]">
        <DialogHeader>
          <DialogTitle>
            Enviar {seleccionadas.length} licencia(s) · {correos} correo(s)
          </DialogTitle>
        </DialogHeader>
        ...
        {correos > AVISO_CUOTA && (
          <p className="flex items-start gap-1.5 rounded-xl bg-amber-50 p-3 text-xs text-amber-800 dark:bg-amber-500/10 dark:text-amber-200">
            ...
            <span>
              ...
              bajan mucho; si no, pártelo en dos días.
            </span>
          </p>
        )}
        ...
        <div className="-mx-4 -mb-4 flex flex-col-reverse gap-2 rounded-b-xl border-t bg-zinc-50 p-4 sm:flex-row sm:justify-end dark:border-zinc-800 dark:bg-zinc-900/60">
          ...
          >
            <X className="mr-1 inline h-4 w-4" /> Cerrar
          </button>
          <button
            type="button"
